$(function () {
    ...
    function advanceSlide() {
        ...
        $('.screenshot').fadeOut(animationDelay, function () {
            $('.screenshot').css('background', 'url(assets/img/' + slide.screenshot + ') top left / cover no-repeat');
        ...
        $('.overlay').fadeTo('slow', 0.0, function () {
            $('.overlay').css('background', 'url(assets/img/' + slide.overlayImage + ') 0 50% / cover no-repeat');
        }).fadeTo('slow', 0.06);
        currentSlide++;
    }
